...
export const MemberCatalog = ({ country }) => {
  ...
  if (isLoading) {
    return <LoadingSpinner message="Loading catalog restaurants..." />;
  }
  ...
  if (selectedRes) {
    return (
      <div className="space-y-4">
        ...
        {isMenuLoading ? (
          <LoadingSpinner message="Fetching delicious items..." />
        ) : isMenuError ? (
          <ErrorMessage
  ...
  if (restaurants.length === 0) {
    return (
      <EmptyState
        ...
        description={`There are currently no restaurants available in ${country}.`}
      />
    );
  }
      ...
      {restaurants.map((res) => (
        ...
        >
          ...
          <div className="pt-2.5 border-t border-[var(--border-subtle)] flex items-center justify-between text-[11px] text-[var(--text-muted)]">
            <span>{res.menu?.length ?? 0} Dishes</span>
